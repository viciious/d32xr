...
====================
=
= PS_SightTraverseIntercepts
=
= Returns true if the traverser function returns true for all lines
====================
*/
boolean PS_SightTraverseIntercepts(sightWork_t *sw)
{
   int count;
   fixed_t dist;
   intercept_t *scan, *in;
...
boolean PS_SightPathTraverse(sightWork_t *sw)
{
   ...
   int count;
   fixed_t x1 = strace->x << FRACBITS, y1 = strace->y << FRACBITS;
   fixed_t x2 = sw->t2x << FRACBITS, y2 = sw->t2y << FRACBITS;
   ...
   strace->x = x1 >> FRACBITS;
   strace->y = y1 >> FRACBITS;
   strace->dx = (x2 - x1) >> FRACBITS;
   strace->dy = (y2 - y1) >> FRACBITS;
   ...
   for (count = 0; count < 64; count++)
   {
      ...
      if (!PS_SightBlockLinesIterator(sw, mapx, mapy))
      ...
      case 3: // xintercept and yintercept both match
         ...
         // be checked.
         if (!PS_SightBlockLinesIterator(sw, mapx + mapxstep, mapy) ||
             !PS_SightBlockLinesIterator(sw, mapx, mapy + mapystep))
            return false;
   ...
   //
traverse:
   return PS_SightTraverseIntercepts(sw);
}
